                        ...
                        foreach $suffix_value (@{$suffix_hash{$suffix}}) {
                           ...
                           if ( exists($hash_AC{"$cat_a"." "."$cat_c"}) ) {
                              if ( exists($hash_BC{"$cat_b"." "."$cat_c"}) ) {
                                 ...
                                 $cnt++; push (@solutions, "$pos_a$pos_b$pos_c\n");
                              }
                           }
                        }
                     }
   ...
   while (<IN>) {
      if (m/^;; /) {  
         ...
         # (2) by deduction: use the $cat (and sometimes the $voc and $gloss) to deduce the appropriate POS
            ...
            elsif  ($cat  =~ m/^F/)          {$POS = "$voc/T"}
            elsif  ($cat  =~ m/^IV/)         {$POS = "$voc/VI"}
            elsif  ($cat  =~ m/^PV/)         {$POS = "$voc/VPR"}
            elsif  ($cat  =~ m/^CV/)         {$POS = "$voc/VIM"}
            elsif (($cat  =~ m/^N/)
              and ($gloss =~ m/^[A-Z]/))     {$POS = "$voc/N"} # educated guess (99% correct)
            elsif (($cat  =~ m/^N/)
              and  ($voc  =~ m/iy~$/))       {$POS = "$voc/N"} # (was NOUN_ADJ: some of these are really ADJ's and need to be tagged manually)
            elsif  ($cat  =~ m/^N/)          {$POS = "$voc/N"}
